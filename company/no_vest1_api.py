...
def detect_frame(stream_name, rtsp_url, frame, conf):
    ...
    for m in results:
        ...
        if has_high_conf:
            ...
            alarm_filename = f"{stream_name}-{alarm_time}-no_vest.jpg"
            alarm_filepath = f"/no_vest/alarmpic/{alarm_filename}"

            video_filename = f"{stream_name}-{alarm_time}-no_vest.mp4"
            video_output_file = f"/no_vest/alarmvideo/{video_filename}"
...
if __name__ == '__main__':
    ...
    # 启动 Flask 服务器
    app.run(host='0.0.0.0', port=1005, debug=False)
