...
def map_box_from_crop_to_image(crop_box_xyxy, det_box_xyxy_in_crop, orig_img_shape, crop_size):
    # crop_box_xyxy: [x1,y1,x2,y2] in original image coords
    # det_box_xyxy_in_crop: [x1,y1,x2,y2] in crop image coords (pixel coords of resized crop)
    # orig_img_shape: (H, W)
    # crop_size: (crop_w, crop_h) used to resize crop for model input
...
if __name__ == "__main__":
    model_path = "company/best.pt"  # 替换为你的模型路径
    model = YOLO(model_path)

    input_directory = r"D:\dingdingDownloads\phone_data_own\val\images"
    output_directory = r"D:\dingdingDownloads\output4"
